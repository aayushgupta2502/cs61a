...
def picky_piggy(score):
    ...
    if score == 0 :
        ...
        points = (int)(((1/7) * (10 ** score) % 10) // 1) # ask about int function
        return points
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1) :
        ...
        jump = current_score - last

        if jump > high :
            high = jump
            print(str(jump) + " point(s)! That's a record gain for Player " + str(who) + "!")
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_score = 0
    best_val = 1
    while highest_val <= 10:
        averaged_roll = make_averaged(roll_dice, trials_count)
        new_score = averaged_roll(highest_val, dice)
        if new_score > best_score:
            best_score = new_score
            best_val = highest_val
        highest_val += 1
    return best_val
