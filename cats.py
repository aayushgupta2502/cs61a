...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    can_be_selected = [paragraph for paragraph in paragraphs if select(paragraph)==True]
...
def accuracy(typed, reference):
    ...
    if len(typed_words) > len(reference_words):
        difference = len(typed_words) - len(reference_words)
        while i<len(reference_words):
            if typed_words[i] == reference_words[i]:
                matches += 1
            i+=1
    return (matches/typed_length) * 100
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    lowest_diff = diff_function(valid_words[0], typed_word, limit)
    closest_word = valid_words[0]
    for word in valid_words:
        if word==typed_word:
            return typed_word
        else:
            if diff_function(word, typed_word, limit) < lowest_diff:
                lowest_diff = diff_function(word, typed_word, limit)
                closest_word = word
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit<0:
        ...
        if start[0] == goal[0]:
            ...
            current = 1
        substitute = current + minimum_mewtations(start[1:], goal[1:], limit-current)
    best_option = min(add, remove, substitute)
    return best_option
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    while counter < len(sofar) :
        if sofar[counter] == prompt[counter]:
            wordsCorrect += 1
            counter += 1
        else :
            break

    progress = wordsCorrect / len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    for times in times_per_player:
        ...
        for i in range(len(words)):
            player_time = times[i+1] - times[i]
            player_time_list.append(player_time)
        diff_list.append(player_time_list)
    return match(words, diff_list)
...
def fastest_words(match):
    ...
    for word in word_indices :
        fastestPlayer = 0
        index0 = time(match, 0, word)
        for player in player_indices :
            if time(match, player, word) < index0 :
                fastestPlayer = player
                index0 = time(match, player, word)

        players[fastestPlayer] += [word_at(match, word)]
